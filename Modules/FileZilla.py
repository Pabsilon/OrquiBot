...
def userLogIn(bot, update, args):
    #Allows a user from the ftp to log in.
    try:
        user = args[0]
        if LOGGED_USERS.get(update.message.from_user.username):
            ...
            for child in root:
                if args[0] == child.get('Name'):
                    exists = True
                    options = child[2]
                    if options.text == 'Users' or  'SubAdmin':
                        LOGGED_USERS[update.message.from_user.username]=args[0]
                        update.message.reply_text('Yay! ' + args[0] + ' logged as ' + update.message.from_user.username)
                    else:
                        update.message.reply_text('I don\'t know any users with that name...')
            if not exists:
                update.message.reply_text('I don\'t know any users with that name...')
        else:
            update.message.reply_text('XML file missing. Try updating Config.py')
        ...
        update.message.reply_text('Usage: /userLogIn Username')